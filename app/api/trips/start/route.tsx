...
export async function POST(request: Request) {
  ...
  try {
    ...
    const [activeTrip] = await db
      ...
      .where(
        and(
          eq(trips.vehicleId, parsedBody.data.vehicleId),
    ...
    if (parsedBody.data.items.length > 0) {
      ...
      const stockByItemId = new Map(
        requestedItems.map((item) => [item.id, item.quantity]),
      );
      ...
      for (const requestedItem of parsedBody.data.items) {
        ...
        if (
          ...
          availableQuantity < requestedItem.quantityTaken
        ) {
          return Response.json(
        ...
        const [updatedItem] = await db
          ...
          .where(
            and(
              ...
              gte(items.quantity, item.quantityTaken),
            ),
          )
